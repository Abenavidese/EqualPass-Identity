...
const MOCK_BADGES: Record<
  string,
  { owner: string; badgeType: number; issuedAt: number; issuer: string } | null
> = {
  "1": {
...
export const VerifierForm: React.FC = () => {
  ...
  const verifyToken = async () => {
    ...
    else setError("Badge no encontrado. Prueba 1, 2 o 3");
    setLoading(false);
  };
      ...
      <div className="mb-3 flex gap-2">
        <button
          className={`px-3 py-1 rounded ${
            mode === "token" ? "bg-gradient-to-r from-indigo-500 to-pink-500 text-white" : "bg-gray-100"
          }`}
        ...
        <button
          className={`px-3 py-1 rounded ${
            mode === "address" ? "bg-gradient-to-r from-indigo-500 to-pink-500 text-white" : "bg-gray-100"
          }`}
      ...
      {mode === "token" ? (
        <div className="space-y-3">
          <Input
            ...
            value={tokenId}
            onChange={(e) => setTokenId((e.target as HTMLInputElement).value)}
          />
          <Button onClick={verifyToken} loading={loading}>
            Verificar Badge
          </Button>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          />
          <Button onClick={verifyAddress} loading={loading}>
            Verificar Dirección
          </Button>
        </div>
      )}
        ...
        {result && (
          <div className="p-3 bg-emerald-50 border border-emerald-100 rounded">
            ...
            <div className="text-sm">
              Propietario: <code className="font-mono text-xs break-all">{result.owner}</code>
            </div>
          </div>
        )}
        {error && <div className="p-3 bg-rose-50 border border-rose-100 rounded text-sm">{error}</div>}
      </div>
    </div>
  );
};
